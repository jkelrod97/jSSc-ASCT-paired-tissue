...
# macro_mono_combined[["DSB"]] <- macro_mono_combined[["DSB_integrated"]] <- macro_mono_combined[["ADT"]] <- NULL
# 
# # Save as H5
...
# Save GSEA results as CSVs
write_csv(res_pbmc$DOWN@result, paste0(project_path, "../to_share/gsea_csvs/pbmc_down_pathways.csv"))
write_csv(res_pbmc$UP@result, paste0(project_path, "../to_share/gsea_csvs/pbmc_up_pathways.csv"))
write_csv(res_skin$DOWN@result, paste0(project_path, "../to_share/gsea_csvs/skin_down_pathways.csv"))
write_csv(res_skin$UP@result, paste0(project_path, "../to_share/gsea_csvs/skin_up_pathways.csv"))
...
# SaveH5Seurat(monocytes_pbmc, paste0(project_path, "data/h5seurat/PBMC/monocytes_pbmc_2"))
# 
...
#               assays = "RNA")
# # Save this UMAP/clustering
# SaveH5Seurat(macrophages_skin, paste0(project_path, "data/h5seurat/skin/macrophages_skin_2"))

# Load in sub-clustered Seurat object
monocytes_pbmc <- LoadH5Seurat(paste0(project_path, "data/h5seurat/PBMC/monocytes_pbmc_2.h5seurat"))
macrophages_skin <- LoadH5Seurat(paste0(project_path, "data/h5seurat/skin/macrophages_skin_2.h5seurat"))
...
macrophages_healthy <- subset(macrophages_skin, health == "healthy")
macrophages_SSc <- subset(macrophages_skin, health == "SSc")

macrophages_healthy <- macrophages_healthy %>% sub_cluster(res = 0.5, assays = "RNA")
healthy_cd14_mac <- FeaturePlot(macrophages_healthy, "CD14", label = T, order = T, reduction = "umap")
healthy_cd16_mac <- FeaturePlot(macrophages_healthy, "FCGR3A", label = T, order = T, reduction = "umap")

macrophages_SSc <- macrophages_SSc %>% sub_cluster(res = 0.5, assays = "RNA")
SSc_cd14_mac <- FeaturePlot(macrophages_SSc, "CD14", label = T, order = T, reduction = "umap")
SSc_cd16_mac <- FeaturePlot(macrophages_SSc, "FCGR3A", label = T, order = T, reduction = "umap")
```
...
SSc_macrophage_rna_markers <- FindAllMarkers(macrophages_SSc, assay = "RNA", group.by = "seurat_clusters", only.pos = T)
