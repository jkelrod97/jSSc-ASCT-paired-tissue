...
In this notebook, we perform preprocessing and QC on our PBMC CITE-seq data. Multimodal clustering and cell type annotation are performed in a follow-up notebook, 2_pbmc_cell_type_annotation.Rmd. 
...
# cellranger_paths <- list.files(paste0(project_path, "data/cellranger_output/cellranger_multi_demultiplexed"), full.names = TRUE)
# 
# # Get sample names
# samp_names <- list.files(paste0(project_path, "data/cellranger_output/cellranger_multi_demultiplexed"))
# 
# # Read 10x output
...
# 
# # Save as RDS files
# mapply(saveRDS, gex_list, paste0(project_path, "data/RDS_files/counts/gex/", samp_names, "_gex.RDS"))
# mapply(saveRDS, plx_list, paste0(project_path, "data/RDS_files/counts/plx/", samp_names, "_plx.RDS"))

# Read in RDS files
gex_paths <- list.files(paste0(project_path, "data/RDS_files/counts/gex"), pattern = "\\.RDS$", full.names = TRUE)
plx_paths <- list.files(paste0(project_path, "data/RDS_files/counts/plx"), pattern = "\\.RDS$", full.names = TRUE)
...
colnames(scler_pbmc_all) <- colnames(scler_pbmc_all) %>%
  ...
  gsub("H[0-9]+_", "H-", .)                           # healthy: H1_, H2_ → H-
...
SaveH5Seurat(scler_pbmc,
             paste0(project_path, "data/h5seurat/scler_pbmc_1"))
```
...
scler_pbmc <- LoadH5Seurat(paste0(project_path, "data/h5seurat/scler_pbmc_1.h5seurat"))
...
# Write to mtx files
rna %>% writeMM(file = paste0(project_path, "data/composite/RNA.mtx"))
adt %>% writeMM(file = paste0(project_path, "data/composite/ADT.mtx"))
```
...
composite_path <- paste0(project_path, "data/composite/multiplet_prediction.csv")
...
# Save as H5
SaveH5Seurat(scler_pbmc,
             paste0(project_path, "data/h5seurat/scler_pbmc_2"))
```


2) Ambient RNA removal with SoupX. Since this step addresses technical noise, we run it separately for each library (subject), so we need output from cellranger multi without demultiplexing (which was run separately for this purpose in 0_C_alignment_without_demultiplexing.sh). 

```{r}
# Path where pooled cellranger multi outputs are saved
path_to_cellranger_denoise <- paste0(project_path, "data/cellranger_output/cellranger_count/cellranger_pooled_for_denoising")

# Load (demultiplexed) Seurat object
scler_pbmc <- LoadH5Seurat(paste0(project_path, "data/h5seurat/scler_pbmc_2.h5seurat"))

# Subjects
subjects <- list.files(paste0(path_to_cellranger_denoise, "raw"))

# Load data for raw and filtered wells
raw_wells <- lapply(subjects, function(subject){Read10X(paste0(path_to_cellranger_denoise, "raw/", subject))})
filtered_wells <- lapply(subjects, function(subject){Read10X(paste0(path_to_cellranger_denoise, "filtered/", subject))})
...
# Save as H5
SaveH5Seurat(scler_pbmc, paste0(project_path, "data/h5seurat/scler_pbmc_3"))
```
...
scler_pbmc <- LoadH5Seurat(paste0(project_path, "data/h5seurat/scler_pbmc_3.h5seurat"))
...
VlnPlot(scler_pbmc, features = c("isotype_ctl_max"), pt.size = 0, group.by = "sample_id")
# Save Seurat object
SaveH5Seurat(scler_pbmc, paste0(project_path, "data/h5seurat/scler_pbmc_4"))
...
We only consider expression in patients, as we do not include healthy controls in our longitudinal model.

```{r}
# Load Seurat object
scler_pbmc <- LoadH5Seurat(paste0(project_path, "data/h5seurat/scler_pbmc_4.h5seurat"))
...
# Save H5
SaveH5Seurat(scler_pbmc, paste0(project_path, "data/h5seurat/scler_pbmc_5"))
```
...
```{r}
# Load Seurat object
scler_pbmc <- LoadH5Seurat(paste0(project_path, "data/h5seurat/scler_pbmc_5.h5seurat"))
...
# Save as H5
SaveH5Seurat(scler_pbmc, paste0(project_path, "data/h5seurat/scler_pbmc_6"))
```
...
For this step, we use the output from cellranger multi without demultiplexing (as we did when we ran SoupX for the ambient RNA correction).

```{r}
# Path where cellranger_count outputs are saved
path_to_cellranger_denoise <- paste0(project_path, "data/cellranger_output/cellranger_pooled_for_denoising/")

# Load (demultiplexed) Seurat object
scler_pbmc <- LoadH5Seurat(paste0(project_path, "data/h5seurat/scler_pbmc_6.h5seurat"))
...
dsb_plots <- mapply(plot_dsb, raw_wells, filtered_wells,
       SIMPLIFY = F)
# Name accordingly
names(dsb_plots) <- subjects

# Get data classifying cells as background/cell
dsb_data <- mapply(dsb_md, raw_wells, filtered_wells,
                   SIMPLIFY = F)
# Name accordingly
...
adt_adj <- mapply(run_dsb,
                  raw_wells, filtered_wells, seurat_subsets, libraries,
...
SaveH5Seurat(scler_pbmc, paste0(project_path, "data/h5seurat/scler_pbmc_7"))
```
...
```{r}
scler_pbmc <- LoadH5Seurat(paste0(project_path, "data/h5seurat/scler_pbmc_7.h5seurat"))
...
SaveH5Seurat(scler_pbmc, paste0(project_path, "data/h5seurat/scler_pbmc_8"))
```
